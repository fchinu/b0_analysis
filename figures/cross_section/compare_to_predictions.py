...
if __name__ == '__main__':
    ...
    gmvfns_mt_sacot_file = ROOT.TFile.Open('gmvfns/gmvfns_pred_13dot6_v1.root')
    g_pred_gmvfns_mt_sacot = gmvfns_mt_sacot_file.Get('gBhadr')
    g_pred_gmvfns_mt_sacot.Scale(1.e3)
    gmvfns_mt_sacot_file.Close()

    kt_file = ROOT.TFile.Open('gmvfns/gmvfns_pred_13dot6_v2.root')
    g_pred_kt = kt_file.Get('gBhadr')
    g_pred_kt.Scale(1.e3)
    kt_file.Close()
    ...
    h_frame = pad_cross_sec.DrawFrame(1, 3.e4, 23.5, 4.e7, ';#it{p}_{T} (GeV/#it{c});d^{2}#sigma/d#it{p}_{T}d#it{y} (pb #kern[-0.5]{#it{c}} / GeV)')
    ...
    h_frame_ratio_fonll = pad_ratio_fonll.DrawFrame(1, 0.2, 23.5, 1.8, ';#it{p}_{T} (GeV/#it{c});Data/FONLL_{ }')
    h_frame_ratio_fonll.GetXaxis().SetTitleOffset(1.1)
    h_frame_ratio_fonll.GetYaxis().SetTitleOffset(0.4)
    h_frame_ratio_fonll.GetXaxis().SetTitleSize(0.12)
    h_frame_ratio_fonll.GetYaxis().SetTitleSize(0.13)
    ...
    h_frame_ratio_gmvfns = pad_ratio_gmvfns.DrawFrame(1, 0.2, 23.5, 1.8, ';#it{p}_{T} (GeV/#it{c});Data/mod-#mu_{#lower[-0.2]{R,F} }')
    h_frame_ratio_gmvfns.GetXaxis().SetTitleOffset(1.1)
    h_frame_ratio_gmvfns.GetYaxis().SetTitleOffset(0.4)
    h_frame_ratio_gmvfns.GetXaxis().SetTitleSize(0.12)
    h_frame_ratio_gmvfns.GetYaxis().SetTitleSize(0.13)
    ...
    h_frame_ratio_gmvfns_mt_sacot = pad_ratio_gmvfns_mt_sacot.DrawFrame(1, 0.2, 23.5, 1.8, ';#it{p}_{T} (GeV/#it{c});#lower[0.15]{Data/SACOT #it{m}_{T }}')
    h_frame_ratio_gmvfns_mt_sacot.GetXaxis().SetTitleOffset(1.1)
    h_frame_ratio_gmvfns_mt_sacot.GetYaxis().SetTitleOffset(0.4)
    h_frame_ratio_gmvfns_mt_sacot.GetXaxis().SetTitleSize(0.12)
    h_frame_ratio_gmvfns_mt_sacot.GetYaxis().SetTitleSize(0.13)
    ...
    h_frame_ratio_kt = pad_ratio_kt.DrawFrame(1, 0.2, 23.5, 1.8, ';#it{p}_{T} (GeV/#it{c});#lower[0.15]{Data/#it{k}_{T} fact._{ }}')
    h_frame_ratio_kt.GetXaxis().SetTitleOffset(1.1)
    h_frame_ratio_kt.GetYaxis().SetTitleOffset(0.4)
    h_frame_ratio_kt.GetXaxis().SetTitleSize(0.12)
    h_frame_ratio_kt.GetYaxis().SetTitleSize(0.13)
    ...
    h_frame_ratio_tamu = pad_ratio_tamu.DrawFrame(1, 0.2, 23.5, 1.8, ';#it{p}_{T} (GeV/#it{c});Data/TAMU_{ }')
    h_frame_ratio_tamu.GetXaxis().SetTitleOffset(1.1)
    h_frame_ratio_tamu.GetYaxis().SetTitleOffset(0.4)
    h_frame_ratio_tamu.GetXaxis().SetTitleSize(0.12)
    h_frame_ratio_tamu.GetYaxis().SetTitleSize(0.13)

...
void simulateBtoDKa(int pdgB = 511, int nDecays = 1000000, int seed = 42) {
  ...
  TFile* massFile = TFile::Open("fit/low_pt_high_pt_shift_template/B0_mass23_24_full_dataset.root");
  TH1D* hWidth = (TH1D*)massFile->Get("h_sigmas");
  hWidth->SetDirectory(0);
  massFile->Close();

  auto tree = new TNtuple("treeBtoDKa", "treeBtoDKa", "mB:mBSmeared:mBcorr:mBcorrSmeared:pTB:pTKa");
  ...
  for (auto iDecay{0}; iDecay < nDecays; ++iDecay) {
    ...
    tree->Fill(pVecB.M(), massBSmeared, pVecBcorr.M(), massBCorrSmeared, pVecBcorr.Pt(), pVecKaCorr.Pt());
  }

  TFile ouputFile(Form("simulations/b_to_dka/MDPi_%d_from_DKa_decay.root", pdgB), "recreate");
  tree->Write();
  hFonll->Write();
  ouputFile.Close();
}